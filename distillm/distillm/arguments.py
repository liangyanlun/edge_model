...
def _escape_percent_in_argparse_help(parser):
    """
    argparse 会对 help 字符串做 `% params` 格式化。
    这里把非 `%(...)` 的 `%` 统一转义为 `%%`，避免 `--help` 崩溃。
    """
    for action in getattr(parser, "_actions", []):
        if not getattr(action, "help", None):
            continue
        # 保留 argparse 的映射占位符：%(default)s / %(type)s ...
        action.help = re.sub(r"%(?!\()", "%%", action.help)
...
def add_peft_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated module name suffixes for LoRA (e.g. q_proj,k_proj,v_proj,o_proj). If omitted, defaults are inferred from the model.",
    )
...
def get_args():
    ...
    _escape_percent_in_argparse_help(parser)
