...
def main():
    ...
    # Historical scripts use `--dev-num` (count). Some users pass `--dev-ratio`
    # (fraction) instead; previously that produced train-only outputs.
    dev_num = args.dev_num
    if dev_num <= 0 and args.dev_ratio is not None and 0 < args.dev_ratio < 1:
        dev_num = max(1, int(len(raw_data) * args.dev_ratio))
