...
def generate(args, tokenizer, model, dataset, device):
    ...
    if get_rank() == 0:
        ...
        for idx, g in zip(all_idxs, all_gen_strs):
            ...
            # For SeqKD-style pseudo data, downstream processing expects the
            # teacher-generated response under the "output" key.
            if "output" in dataset.origin_data[idx]:
                dataset.origin_data[idx]["reference_output"] = dataset.origin_data[idx]["output"]
            dataset.origin_data[idx]["output"] = g
