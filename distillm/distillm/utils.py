...
def get_model(args, device):
    ...
    if args.model_parallel:
        ...
        if args.peft is not None:
            if args.peft == "lora":
                def _infer_lora_target_modules():
                    if getattr(args, "peft_target_modules", None):
                        return [m.strip() for m in args.peft_target_modules.split(",") if m.strip()]

                    # Common transformer projection module suffixes.
                    candidates = [
                        # Attention
                    ...
                    # Filter candidates to only those that exist in the model.
                    found = set()
                    for name, _module in model.named_modules():
                        for cand in candidates:
                            if name.endswith(f".{cand}"):
                                found.add(cand)

                    # If nothing matched (unusual), fall back to a safe default.
                    if found:
                        return sorted(found)
                ...
                if args.peft_path is not None:
                    if args.do_train:
                        ...
                        peft_config = LoraConfig(
                            ...
                            target_modules=_infer_lora_target_modules(),
                        )
...
def get_tokenizer(args):
    ...
    if args.model_type in ["gpt2", "opt", "llama", "gptj", "llama2", "mistral"]:
        ...
        # Keep tokenizer defaults for Qwen-family models (token ids vary across
        # releases). Ensure `pad_token_id` exists for batching.
