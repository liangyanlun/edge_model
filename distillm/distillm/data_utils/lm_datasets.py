...
class LMTrainDataset(Dataset):
    ...
    def _process_lm(self, i, samp, model_data, no_model_data, gen_data):
        ...
        prompt = None
        # `tools/process_data_dolly.py` inserts a `-1` separator between prompt
        # and response. Depending on the binary dtype, it becomes:
        # - uint16: 65535
        # - uint32: 4294967295
        # For Qwen we use uint32 to support large vocab, so we must detect
        # 4294967295; otherwise prompt masking and `--student-gen` break.
        sep_token = 4294967295 if self.args.model_type == "qwen" else 65535
        if sep_token in input_ids:
            source_len = np.where(input_ids == sep_token)[0][0]
